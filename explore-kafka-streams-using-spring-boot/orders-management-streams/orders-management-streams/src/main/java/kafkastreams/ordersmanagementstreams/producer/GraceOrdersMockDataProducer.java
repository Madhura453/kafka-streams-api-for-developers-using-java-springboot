package kafkastreams.ordersmanagementstreams.producer;
...
import kafkastreamsbase.ordersdomain.domain.Order;
import kafkastreamsbase.ordersdomain.domain.OrderLineItem;
...
import static kafkastreams.ordersmanagementstreams.producer.ProducerUtil.publishMessageSync;

@Slf4j
public class GraceOrdersMockDataProducer {
    ...
    public static void main(String[] args) throws InterruptedException {
       ...
       // publishBulkOrders(objectMapper);
         ...
         *
         */
       // publishOrdersToTestGrace(objectMapper, buildOrdersToTestGrace("2023-02-27T21:40:58"));
...
//        publishExpiredRecords(objectMapper);
    ...
    private static List<Order> buildOrdersToTestGrace(String time) {
        var orderItems = List.of(
                new OrderLineItem("Bananas", 2, new BigDecimal("2.00")),
                new OrderLineItem("Iphone Charger", 1, new BigDecimal("25.00"))
        );
        ...
        orders
                .forEach(order -> {
                    try {
                        ...
                        var recordMetaData = publishMessageSync(OrdersTopology.ORDERS_TOPIC, order.orderId() + "", ordersJSON);
